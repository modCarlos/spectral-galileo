...
def get_insider_confidence_adjustment(insider_data):
    ...
    # Heavy insider selling (warning) - optimized via grid search
    elif sentiment == 'BEARISH' and net_value < -8e6:  # > $8M net selling (was $10M)
        return -10, f"Heavy insider selling: ${abs(net_value)/1e6:.1f}M net"
    
    # Moderate insider selling - optimized via grid search
    elif sentiment == 'BEARISH' and net_value < -3e6:  # > $3M net selling (was $5M)
